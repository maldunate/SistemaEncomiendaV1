...
public class AdministrarCamionesController extends Application {
    ...
    @FXML
    void handlerAgregar() {
    	if(!selected && isInteger(capacidad.getText())){
    		SistemaEncomienda.getInstance().getSucursalAPartirDeNombre(SistemaEncomienda.getInstance().getSucursalActual()).agregarCamion(patente.getText(), Integer.parseInt(capacidad.getText()), SistemaEncomienda.getInstance().getSucursalAPartirDeNombre(sucursalDestino.getValue().toString()));
